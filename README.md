...
India's first weekly parametric income shield for Swiggy & Zomato delivery partners.
...
When heavy rain, extreme heat, severe AQI, or a government bandh prevents delivery partners from working, GigShield automatically detects the disruption and pays them Rs.240–480 within 2 hours — no paperwork, no claim forms.
...
3. Paste and run `backend/db/schema.sql`
4. Paste and run `backend/db/zones_seed.sql`
...
# Train the XGBoost model (first time only)
...
| Module | GigShield Simulation |
|--------|---------------------|
| PolicyCenter | `policies` table + policy service |
| ClaimCenter | `claims` table + auto-claim pipeline |
| BillingCenter | Policy renewal API |

*Note: All Guidewire interactions are simulated via local database. Production deployment would call Guidewire REST APIs via the documented endpoints in routers.*
...
*Built for Guidewire DevTrails 2026 Hackathon — April 2026*
